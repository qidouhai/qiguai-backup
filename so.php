...
$data=[];
   ...
   $results=mysqli_query($conn,$sqlu);
	...
	$data[$sign][$geshi]="<font size=5 color=#".randColor().">来自rss结果</font>";;
	 $data[$sign][$i]="<br><a href=".$url.">".$name."</a><br>".$searchresultsf."<br>";
	  }
...
so($searchkeyword);
$dsearchkeyword=strtoupper($searchkeyword);
so($dsearchkeyword);
}elseif(checkcase($searchkeyword)==2){
so($searchkeyword);
$xsearchkeyword=strtolower($searchkeyword);
so($xsearchkeyword);
}elseif(sou($searchkeyword) != null&&sou($searchkeyword) != 1&&sou($searchkeyword) != 2){
so($searchkeyword);
}else{
echo "<center>".$searchkeyword."没有搜到结果</center>";
}
